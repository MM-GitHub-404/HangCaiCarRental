...
<div style="display: none;" id="checkBar">
    ...
    <a class="layui-btn layui-btn-green layui-btn-xs layui-btn-radius" lay-event="exportRent">导出检查单</a>
</div>
...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${alfred}/xxxt_car/check/loadAllCheck' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {fixed: 'right', title: '操作', toolbar: '#checkBar', align: 'center', width: '200'}
            ]]
        });
        ...
        $("#doSearch").click(function () {
            ...
            tableIns.reload({
                url: "${alfred}/xxxt_car/check/loadAllCheck?" + params,
                page: {curr: 1}
            })
        });
        //批量导出
        $("#doExport").click(function () {
            var params = $("#searchFrm").serialize();
            window.location.href="${alfred}/xxxt_car/stat/batchExportCheck?"+params;
        });
        //监听头部工具栏事件
        table.on("toolbar(checkTable)",function (obj) {
            ...
            if (layEvent==='deleteBatch'){
                ...
                layer.confirm('真的要删除这些检查单吗？',function (index) {
                    //向服务端发送删除指令
                    $.post("${alfred}/xxxt_car/check/deleteBatchCheck",params,function (res) {
                        layer.msg(res.msg);
                        tableIns.reload();
                    })
                })
        ...
        table.on('tool(checkTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.checkid + '】这个检查单么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${alfred}/xxxt_car/check/deleteCheck", {checkid: data.checkid}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
                ...
                window.location.href="${alfred}/xxxt_car/stat/exportCheck?checkid="+data.checkid;
            }
        });
        ...
        function openUpdateCheck(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    url = "${alfred}/xxxt_car/check/updateCheck";
                }
            });
        }
