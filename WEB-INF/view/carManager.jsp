...
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'upload'], function () {
        ...
        tableIns = table.render({
            ...
            , url: '${alfred}/xxxt_car/car/loadAllCar' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {
                    field: 'carimg', title: '缩略图', align: 'center', width: '80', templet: function (d) {
                        return "<img width=50 height=30 src=${alfred}/xxxt_car/file/downloadShowFile?path=" + d.carimg + "/>";
                    }
                }
...
//            alert(params);
            tableIns.reload({
                url: "${alfred}/xxxt_car/car/loadAllCar?" + params,
                page: {curr: 1}
            })
        });
...
//            alert(params);
            tableIns.reload({
                url: "${alfred}/xxxt_car/car/esFuzzyQuery?" + params,
                page: {curr: 1}
            })
        });
        //导出
        $("#doExport").click(function () {
            var params = $("#searchFrm").serialize();
            window.location.href = "${alfred}/xxxt_car/stat/exportCar?" + params;
        });
        //监听头部工具栏事件
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除 ' + data.carnumber + ' 这个车辆么？', function (index) {
                    //向服务端发送删除指令
                    $.post("${alfred}/xxxt_car/car/deleteCar", {carnumber: data.carnumber}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCar() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#dataFrm")[0].reset();
                    //设置默认图片
                    $("#showCarImg").attr("src", "${alfred}/xxxt_car/file/downloadShowFile?path=images/defaultcarimage.jpg");
                    $("#carimg").val("images/defaultcarimage.jpg");
                    url = "${alfred}/xxxt_car/car/addCar";
        ...
        function openUpdateCar(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#showCarImg").attr("src", "${alfred}/xxxt_car/file/downloadShowFile?path=" + data.carimg);
                    url = "${alfred}/xxxt_car/car/updateCar";
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些车辆么？', function (index) {
                //向服务端发送删除指令
                $.post("${alfred}/xxxt_car/car/deleteBatchCar", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
        ...
        upload.render({
            elem: '#carimgDiv',
            url: '${alfred}/xxxt_car/file/uploadFile',
            method: "post",  //此处是为了演示之用，实际使用中请将此删除，默认用post方式提交
            acceptMime: 'images/*',
            field: "mf",
            done: function (res, index, upload) {
                $('#showCarImg').attr('src', "${alfred}/xxxt_car/file/downloadShowFile?path=" + res.data.src);
        ...
        function showCarImage(data) {
            mainIndex = layer.open({
                ...
                area: ['750px', '500px'],
                success: function (index) {
                    $("#view_carimg").attr("src", "${alfred}/xxxt_car/file/downloadShowFile?path=" + data.carimg);
                }
            });
        }
